...
> 本文部分命令是早期开发记录；现行产品规则以根目录 `agent.md` 和 `README.md` 为准。真实数据不可静默降级到 Mock。
...
| GET | `/api/family/book.pdf` | 导出本家庭已确认故事的纪念册 PDF（需登录且仍属该家庭） |
...
| POST | `/api/stories/{id}/family-notes/{noteId}/resolve` | 同家庭成员采纳或忽略建议 |
...
curl -X POST http://127.0.0.1:8787/api/agent/interviews -H "Authorization: Bearer <token>" `
  ...
  -d '{"topicId":"hometown","subjectName":"林阿姨","maxRounds":10,"consentVersion":1}'
...
| 家庭协作不越权 | 同一家庭的活跃成员可处理建议和审核故事；代审不改变原讲述者，未处理建议会阻止发布 |
...
4. 后端不可用时真实业务会报错并保留本机已有录音/输入，不能静默填入 Mock 内容（`fallbackToMock: false`）。
...
保存后重启后端。小程序会在一轮录音结束后上传 MP3、提交异步识别任务并轮询结果。云端文字只会预填到校对框，须由家庭成员确认或修改后才能进入故事生成流程。识别失败时保留录音并允许重试或人工填写，不自动注入演示文字。

## 已知边界（演示版）

- 单家庭模型：每个用户首次登录自动建一个家庭；小程序已提供本机邀请码与角色权限演示，真实跨设备加入待上线通信配置。
- 腾讯云 ASR 已接入本机后端；真机纯离线模式仍由讲述者人工校对或使用明确标注的演示转写。
- 一场采访通常引导 7—10 段，讲述者可主动提前结束；每轮录音独立保存并与证据关联。
