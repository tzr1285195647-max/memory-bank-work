...
最近一次代码回归：小程序与后端自动化测试全部通过。原开发电脑的只读演示检查为 **11/12**：当前本机业务库不足两篇已确认故事。队友新克隆后数据库同样为空；不要为凑计数偷偷写入虚构已发布故事。完整规则和验收标准见 [agent.md](agent.md)，架构细节见 [docs/AGENTS_ARCHITECTURE.md](docs/AGENTS_ARCHITECTURE.md)。
