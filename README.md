...
6. 由长辈处理家庭建议并确认故事。新生成的两类故事都经过 LangGraph 人工确认点及故事证据复审；早期无 checkpoint 草稿仍可通过兼容的证据复审入口。真机演示前仍需完整走通一次发布。
7. 打开“故事 → 家庭纪念册”，从封面左右滑动翻页，依次查看目录、按记忆年份升序排列的章节和各成员故事。年份未确认的故事放在末尾；长故事会分页，正文原字保留。内页水彩插图仅作装饰，不描绘或补造讲述事实。
...
- 家庭纪念册现已支持已确认故事的时间线编排、左右翻页、装饰插画和复制全文。书名修改目前只保存在当前设备、按家庭隔离；自动生成与故事内容对应的动态插画、家庭回忆录编辑 Agent、美术编排 Agent、PDF 导出仍未实现。
